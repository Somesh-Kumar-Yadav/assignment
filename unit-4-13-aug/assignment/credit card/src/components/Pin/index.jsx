    ...
    return <>
        ...
        <div style={{display:"flex"}} onPaste={handlePaste}>
            {values.current.map((item, index) => {
                return <Pinitem maxLength={perBox} text={text} length={length*perBox}  ref={(n) => handleCallback(n, index)} handleBackspace={ (value)=>handleBackspace(value,index)} key={index} handleChange={ (value)=>handleChange(value,index)}/>
    })}        
    </div>
    </>
}
